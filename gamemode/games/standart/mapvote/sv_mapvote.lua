...
function MV:FinishMapVote()
	...
	for k,v in pairs(player.GetAll()) do
			umsg.Start( "ChatBroadcastRTVMurder", v)
				umsg.String("Выбрана карта "..win..". Смена через 5 секунд.")
	...
	if args[1] and IsValid( ply ) then
		...
		if IsValid(ply) then
			umsg.Start( "ChatBroadcastRTVMurder", ply)
				umsg.String("Выберите карту")
				umsg.Vector(Vector(255,255,255))
			umsg.End()
			//ply:DeathrunChatPrint("Выберите карту.")
		end
	end
end)
...
concommand.Add("mapvote_nominate_map", function(ply, cmd, args)
	...
	if args[1] then
		...
		if nom == game.GetMap() then
			...
			umsg.Start( "ChatBroadcastRTVMurder", ply)
				umsg.String("Нельзя выдвинуть карту, на которой вы играете.")
				umsg.Vector(Vector(255,255,255))
			umsg.End()
		...
		for k,v in pairs(player.GetAll()) do
			umsg.Start( "ChatBroadcastRTVMurder", v)
			umsg.String(ply:Nick().." "..female.." "..nom.." для голосования!")
			umsg.Vector(Vector(255,255,255))
			umsg.End()
		end
...
function MV:CheckRTV( suppress )
	...
	if ratio > RTVRatio:GetFloat() then
		...
		for k,v in pairs(player.GetAll()) do
			umsg.Start( "ChatBroadcastRTVMurder", v)
			umsg.String("Предел голосов достигнут. Запуск голосования")
			umsg.Vector(Vector(255,255,255))
			umsg.End()
		end
		...
		if not suppress then	
			for k,v in pairs(player.GetAll()) do
				umsg.Start( "ChatBroadcastRTVMurder", v)
				umsg.String(tostring(needed).." осталось для того чтобы сменить карту. Введите !rtv чтобы проголосовать.")
				umsg.Vector(Vector(255,255,255))
			umsg.End()
			end
		end
